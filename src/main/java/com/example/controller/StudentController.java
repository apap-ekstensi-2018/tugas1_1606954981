	...
	public String index ()
    {
	 ...
	 @RequestMapping("/mahasiswa{npm}")
	            ...
	            @RequestParam(value = "npm", required = false) String npm)
	    {
	        ...
	        ProgramStudi prodi = mahasiswaDAO.selectProdi(mahasiswa.getId_prodi());
	        Fakultas fakultas = mahasiswaDAO.selectFakultas(prodi.getId_fakultas());
	        Universitas universitas = mahasiswaDAO.selectUniversitas(fakultas.getId_univ());
    	...
    	@RequestParam(value = "prodi", required = false) Optional<String> prodi)
{
	if (tahun_masuk.isPresent() && prodi.isPresent()) {
		...
		ProgramStudi program_studi = mahasiswaDAO.selectProdi(Integer.parseInt(prodi.get()));
			...
			@RequestParam(value = "idprodi", required = false) String idprodi)
	{
			...
			List<Universitas> universitas = mahasiswaDAO.selectAllUniversitas();
			model.addAttribute ("universitas", universitas);
            	if(univ== null) {
            		return "cari-universitas";
            	} else {
            		int idUniv = Integer.parseInt(univ);
            		Universitas univers = mahasiswaDAO.selectUniversitas(idUniv);
            		...
            		if (idfakultas == null) {
            			...
            			if(idprodi == null) {
            				...
            				ProgramStudi prodis = mahasiswaDAO.selectProdi(idprod);
            				...
            				ProgramStudi prodiSelectObject = mahasiswaDAO.selectProdi(idprod);
